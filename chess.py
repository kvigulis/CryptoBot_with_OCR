...
def chess_trade(coin_label, btc_in_vallet, safety_margin):
    # The input is not case sensitive.
    label = coin_label.lower()
    TradePair = label + "/btc"
    if TradePair in PairDict.values():  # Check if the coin trades on Cryptopia. Remember to update the dictionary.
        ...
        logging.info(' ============ %s  ============ - %s is Cryptopia', get_current_time(), coin_label)
        ...
        before = time.time()
        logging.info(' ============ %s  ============ - Sell order request sent', get_current_time())
        ...
        for order in data:
            ...
            logging.info(' ============ %s  ============ - Sell order Price %s ; Sum %s', get_current_time(), f'{order["Price"]:.8f}', sum)
            #logging.info(get_current_time(), " - Price: ", f'{order["Price"]:.8f}', ";   Sum:", sum)
        ...
        while go_for_it:  # Try buying until, either 'Success: true' or buy price > 170% initial, cheapst price.
            ...
            if buy_price < prices[0] * 1.7:
                print("\nTry a trade at buy price:", f'{buy_price:.8f}', "\n( The initial sell price was: ",
                      f'{prices[0]:.8f}', ")\n\n")
                print("Amount:", buy_amount)
                logging.info(' ============ %s  ============ - Trade order request sent', get_current_time())
                trade_response = ast.literal_eval(api_query("SubmitTrade",
                                                            {"Market": tradePair_underscore, "Type": 'Buy',
                                                             "Rate": buy_price,
                                                             "Amount": buy_amount}))  # Send trade request and convert response string to dictionary.
            else:
            ...
            if trade_response["Success"] == False and retry_counter < 5:
                ...
                print("\n+ + + + + + + + + + + + + + + + + + + + + + + + + + + + + +\n\n")
                logging.info(' ============ %s  ============ - Trade order open price: %s', get_current_time(), f'{buy_price:.8f}')
                print(get_current_time(), "END TIME")
